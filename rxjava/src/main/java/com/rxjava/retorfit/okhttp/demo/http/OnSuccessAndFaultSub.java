package com.rxjava.retorfit.okhttp.demo.http;
...
import com.rxjava.retorfit.okhttp.demo.http.util.CompressUtils;
import com.rxjava.retorfit.okhttp.demo.listener.OnSuccessAndFaultListener;
import com.rxjava.retorfit.okhttp.demo.listener.ProgressCancelListener;
...
public class OnSuccessAndFaultSub extends DisposableObserver<ResponseBody>
     ...
     * @param context 上下文
     */
    public OnSuccessAndFaultSub( Context context, OnSuccessAndFaultListener mOnSuccessAndFaultListener) {
        this.mOnSuccessAndFaultListener = mOnSuccessAndFaultListener;
     ...
     */
    public OnSuccessAndFaultSub(Context context, OnSuccessAndFaultListener mOnSuccessAndFaultListener, boolean showProgress) {
        this.mOnSuccessAndFaultListener = mOnSuccessAndFaultListener;
        this.context = context;
        progressDialog = new ProgressDialog(context);
    ...
    public void onError(Throwable e) {
        try {
            ...
            if (e instanceof SocketTimeoutException) {
                //请求超时
                mOnSuccessAndFaultListener.onFault(-1, "网络连接超时");
            } else if (e instanceof ConnectException) {
                //网络连接超时
                mOnSuccessAndFaultListener.onFault(-1, "网络连接超时");
            } else if (e instanceof SSLHandshakeException) {
                //安全证书异常
                mOnSuccessAndFaultListener.onFault(-1, "安全证书异常");
            } else if (e instanceof HttpException) {
                //请求的地址不存在
                int code = ((HttpException) e).code();
                if (code == 504) {
                    mOnSuccessAndFaultListener.onFault(-1, "网络异常，请检查您的网络状态");
                } else if (code == 404) {
                    mOnSuccessAndFaultListener.onFault(-1, "请求的地址不存在");
                } else {
                    mOnSuccessAndFaultListener.onFault(-1, "请求失败");
                }
            } else if (e instanceof UnknownHostException) {
                //域名解析失败
                mOnSuccessAndFaultListener.onFault(-1, "域名解析失败");
            } else {
                mOnSuccessAndFaultListener.onFault(-1, "error:" + e.getMessage());
            }
        } catch (Exception e2) {
            e2.printStackTrace();
        } finally {
